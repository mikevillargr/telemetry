...
export function DataImportView() {
  ...
  const handleFileUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(`/api/rag/${selectedClient.id}/upload`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      })
      ...
      setUploadResult({ fileName: file.name, chunks: data.data.documentsStored || 0 })
      setStep('summary')
    } catch (err) {
    ...
    <div className="w-full animate-in fade-in duration-300">
      ...
      {step === 'upload' ? (
        <div className="space-y-8 max-w-4xl mx-auto">
          ...
          >
            ...
            {uploading ? (
              ...
              >
                {selectedClient ? 'Browse Files' : 'Select a client first'}
              </Button>
            )}
          </div>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-border bg-card">
                  ...
                  <tr className="hover:bg-accent/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground">Semrush</td>
                    <td className="px-4 py-3 text-muted-foreground">
                      Oct 20, 2025
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-muted-foreground">
        ...
        <div className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-right-4 duration-300">
          ...
          <div>
            ...
            <Card className="shadow-sm overflow-hidden border-border/50 rounded-xl">
              <table className="w-full text-sm text-left">
                <thead className="bg-accent/30 text-muted-foreground border-b border-border/50">
                  <tr>
                    <th className="px-4 py-2.5 font-medium text-xs">Column</th>
          ...
          {/* Actions & AI Input */}
          <div className="pt-6 border-t border-border/50 mt-8 space-y-6">
              ...
              <div className="space-y-3">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm text-muted-foreground bg-accent/20 p-5 rounded-xl leading-relaxed border border-border/40">
                    This appears to be a Google Analytics 4 export containing
                    15,230 rows of session data from Nov 1-30, 2025. It includes
                    dimensions like Source/Medium, Campaign, and Device
                    Category, along with metrics like Sessions, Engagement Rate,
                    and Conversions.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm text-muted-foreground bg-accent/20 p-5 rounded-xl leading-relaxed border border-border/40">
                    ...
                    timeline. It seamlessly connects with the existing Oct 2025
                    data, allowing for MoM trend analysis across all acquisition
                    channels.
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {isEditing ? (
                  ...
                  <p className="text-sm text-muted-foreground bg-accent/20 p-5 rounded-xl leading-relaxed border border-border/40">
                    ...
                    retrieval. Key searchable entities: campaign names, traffic
                    sources, and conversion events. Time-series data will be
                    stored in the structured metrics store for visualization.
                  </p>
                )}
              </div>
            </CardContent>
